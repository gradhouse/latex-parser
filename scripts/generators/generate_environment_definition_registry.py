# File: generate_environment_definition_registry.py
# Description: Generate the environment_definition_registry.json file
#
...
from latex_parser.latex.definitions.register.register_environment_definitions import register_latex_environments


def generate_environment_definition_registry(file_path: str) -> None:
    """
    Generate the environment_definition_registry.json file with LaTeX environment definitions.
...
def main() -> None:
    """
    Main function to generate the environment definition registry.
    """
    if len(sys.argv) != 2:
        print("Usage: python generate_environment_definition_registry.py environment_definition_registry.json")
        print("The project environment definition registry should be moved to the data/latex directory.")
        sys.exit(1)
    ...
    print(f"Generating environment definition registry at: {output_path}")

    try:
        generate_environment_definition_registry(output_path)
        print("Environment definition registry generated successfully!")
    except Exception as e:
        print(f"Error generating environment definition registry: {e}")
        sys.exit(1)
